...
static std::array<Description, ErrorCode::ENUM_END> loadErrorStrings() noexcept {
    try {
        ...
        TERMINATE( "[encoding::ErrorCode::] Failed to load error data (loadErrorStrings): " );
    }
 ...
 */
const std::string & blogator::encoding::ErrorCode::str( int err ) {
 ...
 */
const std::string & blogator::encoding::ErrorCode::detailed( int err ) {
    if( ( err = abs( err ) ) >= ENUM_END ) {
