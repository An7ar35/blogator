...
namespace blogator::parser::specs::infra {
    enum class Attribute {
        ...
        /* MathML attributes */
        MATHML_ATTR_BEGIN,
        MATHML_DEFINITIONURL = MATHML_ATTR_BEGIN,
        MATHML_ATTR_END = MATHML_DEFINITIONURL,

        /* SVG attributes */
        SVG_ATTR_BEGIN,
        SVG_ATTRIBUTENAME = SVG_ATTR_BEGIN, SVG_ATTRIBUTETYPE,
        SVG_BASEFREQUENCY, SVG_BASEPROFILE,
        SVG_CALCMODE, SVG_CLIPPATHUNITS,
        SVG_DIFFUSECONSTANT,
        SVG_EDGEMODE,
        SVG_FILTERUNITS,
        SVG_GLYPHREF, SVG_GRADIENTTRANSFORM, SVG_GRADIENTUNITS,
        SVG_KERNELMATRIX, SVG_KERNELUNITLENGTH, SVG_KEYPOINTS, SVG_KEYSPLINES, SVG_KEYTIMES,
        SVG_LENGTHADJUST, SVG_LIMITINGCONEANGLE,
        SVG_MARKERHEIGHT, SVG_MARKERUNITS, SVG_MARKERWIDTH, SVG_MASKCONTENTUNITS, SVG_MASKUNITS,
        SVG_NUMOCTAVES,
        SVG_PATHLENGTH, SVG_PATTERNCONTENTUNITS, SVG_PATTERNTRANSFORM, SVG_PATTERNUNITS, SVG_POINTSATX, SVG_POINTSATY, SVG_POINTSATZ,
            SVG_PRESERVEALPHA, SVG_PRESERVEASPECTRATIO, SVG_PRIMITIVEUNITS,
        SVG_REFX, SVG_REFY, SVG_REPEATCOUNT, SVG_REPEATDUR, SVG_REQUIREDEXTENSIONS, SVG_REQUIREDFEATURES,
        SVG_SPECULARCONSTANT, SVG_SPECULAREXPONENT, SVG_SPREADMETHOD,
            SVG_STARTOFFSET, SVG_STDDEVIATION, SVG_STITCHTILES,
            SVG_SURFACESCALE, SVG_SYSTEMLANGUAGE,
        SVG_TABLEVALUES, SVG_TARGETX, SVG_TARGETY, SVG_TEXTLENGTH,
        SVG_VIEWBOX, SVG_VIEWTARGET,
        SVG_XCHANNELSELECTOR,
        SVG_YCHANNELSELECTOR,
        SVG_ZOOMANDPAN,
        SVG_ATTR_END = SVG_ZOOMANDPAN,

        ENUM_END = SVG_ATTR_END //used for iterating enums
    };

    std::ostream & operator <<( std::ostream &os, Attribute attr );
    std::u32string adjustMathMLAttribute( const std::u32string & name );
    std::u32string adjustSVGAttribute( const std::u32string & name );
}
