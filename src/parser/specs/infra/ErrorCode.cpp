...
static std::array<Description, ErrorCode::ENUM_END> loadErrorStrings() noexcept {
    try {
        ...
        arr[ErrorCode::INVALID_STARTTAG_TOKEN_IN_FOREIGN_CONTENT] =  {
            "invalid start tag token in foreign content",
            R"(This error occurs when a 'start tag' token with an invalid name is passed as foreign content. The token will reprocessed as HTML content.)"
        };
